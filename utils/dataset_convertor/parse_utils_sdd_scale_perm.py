...
def create_dataset(p_data, t_data, t_range, n_past=8, n_next=12):
    ...
    for t in tqdm(range(t_range.start, t_range.stop, 1)):
        for i in range(len(t_data)):
            ...
            if len(p_data[i][tP_ind:t0_ind:t_range.step]) != n_past or len(p_data[i][t0_ind:tF_ind+t_range.step:t_range.step]) != n_next:
                continue

            dataset_t0.append(t)
            dataset_x.append(p_data[i][tP_ind:t0_ind:t_range.step])
            dataset_y.append(p_data[i][t0_ind:tF_ind+t_range.step:t_range.step])
